...
class BaseInfluencer(ABC):
    ...
    def display_campaigns_participated(self):
        ...
        display_info = f"{self.__class__.__name__} :) {self.username} :) participated in the following campaigns:\n"
        for campaign in self.campaigns_participated:
            display_info += f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, " \
